import { BIN_APP_EXPR, BinOperation, Expr, ExprType, NOT_EXPR, ONE_EXPR_VAL, SHIFT_EXPR, VALUE_EXPR, ZERO_EXPR_VAL, evaluate, prettyPrint } from "./AST";
import { Digit, digitsToInt, intToDigits, assert } from "./Common";
...
function buildExprForSimilarBits(expr: Expr, to: Digit[], maxDifferingBits: number): Expr | null {
    let from = intToDigits(evaluate(expr));
    let differingBits = 0;
    let differingBitsPositions: number[] = [];
